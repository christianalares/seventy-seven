...
export async function GET(request: NextRequest) {
  ...
  const veryfiedState = await slackInstaller.stateStore?.verifyStateParam(new Date(), parsedParams.data.state)
  const parsedMetadata = metadataSchema.safeParse(JSON.parse(veryfiedState?.metadata ?? '{}'))
  ...
  try {
    ...
    if (!parsedJson.success) {
      console.error('Invalid JSON response from slack', parsedJson.error.errors)
    ...
    if (createdSlackIntegration) {
      ...
      return NextResponse.redirect(`${requestUrl.origin}/all-done?event=slack_oauth_completed`)
